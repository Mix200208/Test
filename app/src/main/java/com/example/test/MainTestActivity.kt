...
import com.example.test.ListPost.Companion.NickPhoto
import com.example.test.ListPost.Companion.mdata
...
//        val username: String? = intent.getStringExtra(USER_NAME)
        ...
        imgProfile.setImageResource(NickPhoto[username] ?: R.drawable.test4)

        findViewById<TextView>(R.id.countPost).text =
            mdata.filter { post -> post.nick_info == username }.size.toString()
    ...
    private fun postAdapt() {
        postAdapter = PostAdapter(mdata.filter { post -> post.nick_info == username })
